...
int main() {
    ...
    auto& out_of_range_family = prometheus::BuildCounter()
        ...
        .Help("Total count of inputs outside valid range")
        .Register(*registry);
    ...
    // =========================
    prometheus::Exposer exposer{"0.0.0.0:" + std::to_string(PROMETHEUS_PORT)};
    exposer.RegisterCollectable(registry);
    ...
    if (interactive) {
        ...
        while (!stop_application) {
            ...
            if (!std::getline(std::cin, input_line) || input_line == "quit")
                break;

            try {
                long long n = std::stoll(input_line);
                process_input(n,
                              checked_numbers_total,
